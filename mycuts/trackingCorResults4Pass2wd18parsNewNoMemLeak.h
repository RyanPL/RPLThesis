...
//double *target_info18par(int PID,double xr,double yr,double cxdc,double cydc,double xdc,double ydc, double zdc,double p,int q)
void target_info18par(int PID,double xr,double yr,double cxdc,double cydc,double xdc,double ydc, double zdc,double p,int q, double target[])
{
...
#ifdef DEBUG_TRK_COR  //6/15/17
  cout<<"In trkCor:  :  electronID="<<electronID<<", protonID="<<protonID<<endl;
#endif
...
#ifdef DEBUG_TRK_COR  //6/15/17
      cout<<"check point 1:  czdc, thf, phif:  "<<czdc<<", "<<thf<<", "<<phif<<endl;
#endif
...
#ifdef DEBUG_TRK_COR  //6/15/17
      cout<<"check point 2:  thf:  "<<thf<<endl;
#endif
...
#ifdef DEBUG_TRK_COR  //6/15/17
      cout<<"check point 3:  phi0, dth, xp, yp:  "<<phi0<<", "<<dth<<", "<<xp<<", "<<yp<<endl;
#endif
...
#ifdef DEBUG_TRK_COR  //6/15/17
      cout<<"check point 4:  dth, th0, czc, phi0, cxc, cyc:  "<<dth<<", "<<th0<<", "<<czc<<", "<<phi0<<", "<<cxc<<", "<<cyc<<endl;
#endif
...
#ifdef DEBUG_TRK_COR  //6/15/17
      cout<<"check point 5:  cxc, cyc, czc, th0:  "<<cxc<<", "<<cyc<<", "<<czc<<", "<<th0<<endl;
#endif
...
#ifdef DEBUG_TRK_COR  //6/15/17
      cout<<"check point 6:  rdc, vz:  "<<rdc<<", "<<vz<<endl;
#endif
...
double nwPar21p2[] = //{3.42931e-05, 0.000422835, -4.07979e-05, -0.000420968, 4330.72, 4440.27, -101.022, 4129.42, 4340.07, -101.064,4104.95, 4251.29, -101.041, 4014.65, 3946.27, -101.052, 4192.43, 3343.59, -101.002, 0.00096982, 0.00201097}; //pass1 values 
  {0.000129965, 0.00012713, -4.33035e-05, -0.000413682, 4007.02, 3849.15, -100.649, 3706.94, 3871.33, -100.733, 3634.35, 3817.44, -100.856, 3642.01, 3595.97, -100.818, 4406.01, 3334.81, -100.975, 0.00213645, -2.75496e-05};//Pass2 (11/2/15) https://www.jlab.org/Hall-B//secure/eg4/adhikari/Corrections/Mcor/CheckMomCorEg4/Pass2/Tf21pSep16_00.txt
				...
				double phiPhys, double vZ, double & phicor, double & vZcor, double & xr, double & yr)
{
  ...
  double Xgain = nwPar21p2[0] + nwPar21p2[1]/Ebeam, Ygain = nwPar21p2[2] + nwPar21p2[3]/Ebeam;
  double Xoffset = nwPar21p2[3*Eb_index + 4], Yoffset = nwPar21p2[3*Eb_index + 5]; //Zave=nwPar21p2[3*Eb_index + 6] (see in fcn7par21())
  ...
  double factor1 = int(q)*10.0*(nwPar21p2[19]*Csx+ nwPar21p2[20]*Csy)/(p*sin(theta)*sin(theta));
